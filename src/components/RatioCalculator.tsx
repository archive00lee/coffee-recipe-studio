...
export const RatioCalculator: React.FC<RatioCalculatorProps> = ({
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      {/* Header */}
      <div className="bg-stone-800/80 p-5 rounded-2xl border border-stone-800 shadow-xl flex items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-400">원두량과 추출 방식에 맞는 정밀 물량 및 얼음 비율 산출</p>
          </div>
        </div>
        ...
        >
          <Snowflake className={`w-3.5 h-3.5 ${isIcedMode ? 'text-sky-400' : ''}`} />
          <span>아이스 드립 비율 모드</span>
        </button>
      </div>

      {/* Main Interactive Controls */}
      <div className="bg-stone-800/90 border border-stone-700 p-6 rounded-3xl shadow-2xl space-y-6">
        ...
        {/* Inputs & Sliders */}
        <div className="grid md:grid-cols-2 gap-6 bg-stone-900/80 p-5 rounded-2xl border border-stone-800">
          {/* Bean Grams */}
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold text-stone-300">원두 무게 (g)</label>
              <span className="text-base font-black text-amber-400 font-mono">{beanAmount}g</span>
            </div>
            <input
              type="range"
              min="5"
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold text-stone-300">추출 비율 (1 : N)</label>
              <span className="text-base font-black text-amber-400 font-mono">1 : {ratio}</span>
            </div>
            <input
              type="range"
              min="1"
        ...
        {isIcedMode && (
          <div className="bg-sky-950/40 p-4 rounded-2xl border border-sky-800/60 space-y-2 animate-fade-in">
            <div className="flex justify-between items-center text-xs font-bold text-sky-300">
              ...
              <span className="font-mono text-sky-400">{iceRatioPercent}% 얼음</span>
            </div>
            <input
              type="range"
              min="20"
        ...
        <div className="bg-stone-950 p-6 rounded-2xl border border-stone-800 text-center space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2">
            <div className="bg-stone-900 p-3 rounded-xl border border-stone-800 text-center">
              <div className="text-[10px] text-stone-400 mb-1">원두 용량</div>
            ...
            {isIcedMode ? (
              <>
                <div className="bg-stone-900 p-3 rounded-xl border border-stone-800 text-center">
                  <div className="text-[10px] text-stone-400 flex items-center justify-center gap-1 mb-1">
                    <Flame className="w-3 h-3 text-rose-400" />
                    <span>뜨거운 물</span>
                  </div>
                  <div className="text-lg font-bold text-rose-300 font-mono">{hotWaterGrams}g</div>
                </div>

                <div className="bg-stone-900 p-3 rounded-xl border border-stone-800 text-center col-span-2 sm:col-span-1">
                  <div className="text-[10px] text-stone-400 flex items-center justify-center gap-1 mb-1">
                    <Snowflake className="w-3 h-3 text-sky-400" />
                    <span>서버 준비 얼음</span>
                  </div>
                  <div className="text-lg font-bold text-sky-300 font-mono">{iceGrams}g</div>
                </div>
              </>
            ) : (
              <>
                <div className="bg-stone-900 p-3 rounded-xl border border-stone-800 text-center">
                  ...
                  <div className="text-lg font-bold text-amber-300 font-mono">{totalWater}ml</div>
                </div>

                <div className="bg-stone-900 p-3 rounded-xl border border-stone-800 text-center col-span-2 sm:col-span-1">
                  <div className="text-[10px] text-stone-400 mb-1">권장 수온</div>
                  <div className="text-lg font-bold text-stone-200 font-mono">92 ~ 94°C</div>
                </div>
              </>
            )}
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-amber-600 hover:bg-amber-500 text-white font-semibold text-xs sm:text-sm rounded-xl shadow-lg transition flex items-center justify-center gap-2 mt-2"
          >
      ...
      <div className="bg-stone-800/40 p-4 rounded-xl border border-stone-800 text-xs text-stone-400 flex items-start gap-2.5">
