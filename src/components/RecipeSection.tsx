    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
        <div>
          <h2 className="text-2xl font-black text-white flex items-center gap-2 tracking-tight">
            ...
            <span className="text-xs font-mono px-2.5 py-0.5 rounded-full bg-zinc-800 text-zinc-100 border border-zinc-700">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={openModal}
            className="flex items-center space-x-2 bg-white text-black hover:bg-zinc-200 px-4 py-2 rounded-xl transition font-extrabold text-xs sm:text-sm active:scale-95"
          >
        ...
        <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-9 pr-3 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-600 transition"
            />
          </div>
      ...
      {filteredRecipes.length > 0 ? (
        <div id="recipe-grid" className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRecipes.map((item) => (
            <div
              key={item.id}
              className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 p-5 rounded-2xl flex flex-col justify-between transition-all duration-200 group"
            >
              <div>
                ...
                {/* Ratio Tag */}
                <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 mb-3 font-mono text-xs text-zinc-200 flex items-center justify-between">
                  <span className="truncate">{item.ratioText}</span>
                  <span className="text-[10px] font-sans text-zinc-300 bg-zinc-800 px-2 py-0.5 rounded border border-zinc-700">
                ...
                <div className="grid grid-cols-3 gap-1.5 text-[11px] text-zinc-300 mb-3">
                  <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-1.5 flex items-center gap-1 text-zinc-300">
                    <Thermometer className="w-3 h-3 text-zinc-400 shrink-0" />
                    <span className="truncate">{item.waterTempCelsius}°C</span>
                  </div>

                  <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-1.5 flex items-center gap-1 text-zinc-300 font-mono">
              ...
              <div className="mt-4 pt-3 border-t border-zinc-800 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="text-xs text-white hover:text-black font-extrabold bg-zinc-800 hover:bg-white px-3 py-1.5 rounded-lg border border-zinc-700 hover:border-white transition-all duration-200 flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="text-xs text-zinc-300 hover:text-white font-semibold bg-zinc-800/50 hover:bg-zinc-800 px-2.5 py-1.5 rounded-lg border border-zinc-700 transition-all duration-200 flex items-center gap-1"
                    title="레시피 수정"
                  >
                ...
                <button
                  ...
                  className="text-xs text-zinc-400 hover:text-white transition p-1.5 hover:bg-zinc-800 rounded-lg"
                  title="레시피 삭제"
                >
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-12 text-center space-y-4">
          <div className="w-12 h-12 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center mx-auto text-white">
          ...
          <button
            onClick={openModal}
            className="px-5 py-2.5 bg-white hover:bg-zinc-200 text-black font-extrabold text-xs rounded-xl transition inline-flex items-center gap-1.5"
          >
      ...
      {deletingId !== null && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-800 max-w-sm w-full p-6 rounded-2xl space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-white text-black hover:bg-zinc-200 text-xs font-extrabold rounded-lg transition"
              >
                삭제하기
              </button>
            </div>
